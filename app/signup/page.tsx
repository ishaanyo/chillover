    ...
    <div style={{ minHeight: '100vh', background: '#0a0a0a', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
      <div style={{ background: '#1a1a1a', padding: '3rem', border: '1px solid rgba(245,242,237,0.07)', width: '100%', maxWidth: '440px' }}>
        ...
        <form action={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
          <div>
            <label style={{ display: 'block', fontFamily: 'var(--font-mono)', fontSize: '0.6rem', letterSpacing: '0.1em', textTransform: 'uppercase', color: '#888', marginBottom: '0.4rem' }}>Full Name</label>
            <input name="name" type="text" required style={{ width: '100%', padding: '0.8rem', background: '#111', border: '1px solid rgba(245,242,237,0.1)', color: '#f5f2ed', fontFamily: 'var(--font-mono)', fontSize: '0.8rem' }} />
          </div>
        ...
        <div style={{ marginTop: '2rem', textAlign: 'center', fontFamily: 'var(--font-mono)', fontSize: '0.65rem', letterSpacing: '0.05em', color: '#888' }}>
          Already have an account? <Link href="/login" style={{ color: '#ff3c1e', textDecoration: 'none' }}>Sign In</Link>
        </div>
      </div>
    </div>
  );
